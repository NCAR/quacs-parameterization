...
if __name__ == "__main__":
    ...
    reactions = [
        mc.FirstOrderLoss(
            name=f"{sp.name}_drydep", scaling_factor=1.0, reactants=[sp], gas_phase=gas
        )
        for sp in species
    ]
    mechanism = mc.Mechanism(
        name="drydep_multispecies_box", species=species, phases=[gas], reactions=reactions
    )
    solver = musica.MICM(
    ...
    for step in range(0, n_steps + 1, max(1, n_steps // 20)):
        ...
        row += "  ".join(
            f"{concs[sp.name][step] * sp.molecular_weight_kg_mol * 1e12:14.6f}" for sp in species
        )
        print(row)
