...
df_dd = data_dd_f.loc[
    ~data_dd_f["Litterfall Dry Deposition velocity (cm s-1)"].isna()
].reset_index()
...
IRI[2] = 200.0  # Change RI resistance of coniferous forests to match deciduous, as in GEOS-Chem
...
DV_lt = np.zeros(
    (n_sites, 12)
)  # array of deposition velocity monthly values for each site, corrected by land type
...
for j in range(12):
    ...
    for i in range(n_sites):
        ...
        for LDT in range(n_landtypes):
            ...
            scaling_fac[LDT] = weird_division(1.0, Olson_landtype_v[LDT, lat_i, lon_i])
        ...
        for k in range(time_l):
            ...
            if sum_area_2 > 0:
                XLAI_2_all = (
                    sum(XLAI_old_scale[IOLSON_2] * Olson_landtype_v[IOLSON_2, lat_i, lon_i])
                    / sum_area_2
                )
            # Coniferous
            ...
            if sum_area_3 > 0:
                XLAI_3_all = (
                    sum(XLAI_old_scale[IOLSON_3] * Olson_landtype_v[IOLSON_3, lat_i, lon_i])
                    / sum_area_3
                )
            # Agricultural
            ...
            if sum_area_4 > 0:
                XLAI_4_all = (
                    sum(XLAI_old_scale[IOLSON_4] * Olson_landtype_v[IOLSON_4, lat_i, lon_i])
                    / sum_area_4
                )
            # Rainforest
            ...
            if sum_area_6 > 0:
                XLAI_6_all = (
                    sum(XLAI_old_scale[IOLSON_6] * Olson_landtype_v[IOLSON_6, lat_i, lon_i])
                    / sum_area_6
                )
            ...
            if site_types[i] == 2:
                if sum_area_2 != 0:
                    ...
                    "Devil's Lake, WI, USA" in location[i]
                ):  # exception because no forest for this grid box
            ...
            CZ1, LSNOW, OBK, W10 = METERO(
                bxheight[k, lat_i, lon_i],
...
sio.savemat(
    fn_output,
    {"DV_lt": DV_lt, "F0": F0, "obs_lat": obs_lat, "obs_lon": obs_lon, "site_types": site_types},
)
