...
def GET_OBK(TS, USTAR, AIRDEN, HFLUX):
    ...
    if HFLUX == 0.0:  # make sure denominator not zero
...
def Compute_Olson_landmap(Olson_landtype):
    ...
    ILAND = np.full(
        [NSURFTYPE, lat_l, lon_l], np.nan, dtype=int
    )  # index of types actually in grid box
    IUSE = np.zeros(
        (NSURFTYPE, lat_l, lon_l)
    ...
    ILAND_xr = xr.DataArray(
        ILAND, dims=("variable", "lat", "lon"), coords={"variable": ltype, "lat": lat, "lon": lon}
    )
    IUSE_xr = xr.DataArray(
        IUSE, dims=("variable", "lat", "lon"), coords={"variable": ltype, "lat": lat, "lon": lon}
    )
...
def BIOFIT(COEFF1, XLAI1, SUNCOS1, CFRAC1, NPOLY):
    ...
    BIO_FIT = 0.0  # resultant light correction
...
def SUNPARAM(X):
    ...
    ND = [55.0, 20.0, 11.0]  # scaling factor for each variable

    X0 = [11.0, 1.0, 1.0]  # maximum for each variable

    for I in range(NN):  # noqa: E741
        X[I] = min(X[I], X0[I])
...
def DIFFG(TK, PRESS, XM):
    ...
    Z = XM / XMAIR
    FRPATH = 1.0 / (PI * np.sqrt(1.0 + Z) * AIRDEN * (DIAM**2))

    # Calculate average speed of gas X; eq. 15.47 of Levine [1988]
    SPEED = np.sqrt(8.0 * RSTARG * TK / (PI * XM))
...
def DEPVEL(
    ...
    SMALL = 1e-10  # noqa: F841  # small number to avoid instabilities
    ...
    for LDT in range(IREG):
        ...
        XLAI_scale = XLAI[IOLSON] / IUSE[LDT] * 1000.0
        ...
        if (IRLU[II - 1] >= 9999.0) or (XLAI_scale <= 0.0):
            RLU[LDT] = 1e6
        else:
        ...
        if RIX < 9999.0:
            GFACT = 100.0
            if (TEMPC > 0.0) and (TEMPC < 40.0):
                GFACT = 400.0 / TEMPC / (40.0 - TEMPC)

            GFACI = 100.0
            if (RA > 0.0) and (XLAI_scale > 0.0):
                GFACI = 1.0 / BIOFIT(DRYCOEFF, XLAI_scale, SUNCOSMID, CLDFRC, NN)
        ...
        RDC = 100.0 * (1.0 + 1000.0 / (RA + 10.0))

        # Calculate species dependent dry deposition parameters

        XMWH2O = H2OMW * 1.0e-3
        RIXX = RIX * DIFFG(TEMPK, PRESSU, XMWH2O) / DIFFG(TEMPK, PRESSU, XMW) + 1 / (
            HSTAR / 3000.0 + 100.0 * F0
        )
        ...
        # soil resistance
        RGSX = 1.0 / (HSTAR / 1e5 / RGSS[LDT] + F0 / RGSO[LDT])

        # lower canopy resistance
        RCLX = 1.0 / (HSTAR / 1e5 / RCLS[LDT] + F0 / RCLO[LDT])
        ...
        DTMP1 = 1.0 / RIXX
        DTMP2 = 1.0 / RLUXX
        DTMP3 = 1.0 / (RAC[LDT] + RGSX)
        DTMP4 = 1.0 / (RDC + RCLX)
        RSURFC[LDT] = 1.0 / (DTMP1 + DTMP2 + DTMP3 + DTMP4)

        # Set maximum value of surface resistance
        RSURFC[LDT] = max(1.0, min(RSURFC[LDT], 9999.0))
        ...
        if REYNO >= 0.1:
            ...
            if CORR1 < 0.0:
                # unstable conditions;
                DUMMY1 = (1.0 - 15.0 * CORR1) ** 0.5
                DUMMY2 = (1.0 - 15.0 * Z0OBK) ** 0.5
                DUMMY3 = abs((DUMMY1 - 1.0) / (DUMMY1 + 1.0))
                DUMMY4 = abs((DUMMY2 - 1.0) / (DUMMY2 + 1.0))
                RA = 1.0 * (1.0 / CKUSTR) * np.log(DUMMY3 / DUMMY4)

            elif (CORR1 >= 0.0) and (CORR1 <= 1.0):
                RA = (1.0 / CKUSTR) * (1.0 * np.log(CORR1 / Z0OBK) + 5.0 * (CORR1 - Z0OBK))
            else:  # CORR1 > 1.0
                RA = (1.0 / CKUSTR) * (5.0 * np.log(CORR1 / Z0OBK) + 1.0 * (CORR1 - Z0OBK))
            ...
            RB = (2.0 / CKUSTR) * (DAIR / DIFFG(TEMPK, PRESSU, XMW)) ** 0.667
